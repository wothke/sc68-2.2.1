...
#else
static int buffer_read_68k(unsigned int dest, char * buf, unsigned int sz)
{
  if (EMU68_memvalid(dest, sz)) {
    return SC68error_add("68Kread_stream() : %s",EMU68error_get());
  }
  ...
  int i;
  unsigned char *mem= reg68.mem+dest;
  for(i=0; i<sz; i++) {
	mem[i]= buf[i];
  }
  return 0;
...
#ifdef EMSCRIPTEN
void api68_override_max_playtime(int maxPlaytime) {
	api68->time.def_ms = maxPlaytime;
}
#endif
...
char * getPlayerBuf(char *key);
unsigned int getPlayerBufLen(char *key);

#ifdef ONDEMAND

extern int callback_check_replay(const char* name);	// to be defined in JavaScript as "_callback_check_replay"
static long replay_hash= -1;
static char *replay_buffer= 0;
static int replay_buffer_len= 0;

static long replayHash(char *key, int len) {
    long h= 0;
    int i= 0;
    while ((!len || (i <len)) && key[i] != 0) {
        h ^= key[i++]*i;
    }
    return h;
}

// asynchronously called setter invoked from JavaScript when "replay" binary is available
int api68_set_replay(char *key, int keyLen, char *buf, int len) {
	replay_hash= !(key == 0) ? replayHash(key, keyLen) : -1;
	
	if (replay_buffer) api68_free(replay_buffer);

	replay_buffer= api68_alloc(len);
	int i;
	for (i= 0; i<len; i++) {
		replay_buffer[i]= buf[i];
	}
	replay_buffer_len= len;
 ...
 *
 */
int apply_change_track(api68_t * api)
{
  u32 a0;
  disk68_t * d;
  music68_t * m;
...
#else
	char *playerName= mystrtolower(m->replay, m->replay, strlen(m->replay));
	fprintf(stderr, "loading replay: %s\n", playerName);
#ifndef ONDEMAND
	// just use the inlined data
	char* buf= getPlayerBuf(playerName);
	size= getPlayerBufLen(playerName);
#else
	// replay data must be loaded asynchronously, i.e. we can request the data now but 
	// it may not be available yet, i.e. we'll have come back later to retry
	
	int key= replayHash(playerName, strlen(playerName));
	if (!(replay_hash == key)) {		// suitable replay is not currently loaded
		api->track_to = !api->track_to;				// ensure above init does not prematurely abort in the next call..
		
		api68_set_replay(0,0,0,0);
				
		callback_check_replay(playerName);	// request "replay" data to be sent over from JavaScript
	}
	if (replay_buffer_len == 0) {
		return API68_WAIT_REPLAY;
	}
	
	char* buf= replay_buffer;
	size= replay_buffer_len;
#endif
	if ((strcmp("deltamusic1.bin", playerName) == 0) ||
		(strcmp("deltamusic2.bin", playerName) == 0) ||
		(strcmp("fred.bin", playerName) == 0) ||
		(strcmp("psg1.bin", playerName) == 0) ||
		(strcmp("sidmon1.bin", playerName) == 0) ||
		(strcmp("sidmon2.bin", playerName) == 0) ||
		(strcmp("soundfx.bin", playerName) == 0)) {
		
		api->playaddr = a0 = (!m->a0) ? 0x10000 : m->a0;	// HACK: replays from new version	
	}

	buffer_read_68k(a0, buf, size);
#endif
...
#ifdef EMSCRIPTEN
#ifdef ONDEMAND
	if (ret & API68_WAIT_REPLAY) return ret;
#endif
#endif
  }
...
#ifdef EMSCRIPTEN
#ifdef ONDEMAND
	if (code & API68_WAIT_REPLAY) return code;
#endif
#endif
